...
bool IntroWorld::CreateModelType()
{
	...
	/// 버튼0----------------------------------------------------------------
	std::shared_ptr<ButtonObject> btnObj(new ButtonObject);
	...
	// 새로운 모델을 생성해서 등록한다.
	std::shared_ptr<UIModelComposed> pComposedObj(new UIModelComposed);
...
bool IntroWorld::Load(std::wstring file)
{
	...
	pNewBtn2->SetPosition(Vector2(400, 150));
	pNewBtn2->UpdateData();
	...
	m_UIObj.push_back(std::shared_ptr<Object2D>(pNewDlgBtnClone));

	std::shared_ptr<ListCtrlObject> pListCtrl =std::make_shared<ListCtrlObject>();
