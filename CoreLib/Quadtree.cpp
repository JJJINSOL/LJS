...
{	
	for (auto node : g_pLeafNodes)
	{
		...
		p.z = node->m_Box.vMin.z - node->m_Box.vSize.z;
		node->m_pNeighborList[1] = CheckBoxtoPoint(p); // 남

		p.z = node->m_Box.vCenter.z;
		p.x = node->m_Box.vMin.x - node->m_Box.vSize.x;
...
bool Quadtree::AddObject(MapObject* obj)
{
...
{
	if (pNode == nullptr) return;
	for (std::list<MapObject*>::iterator iter = pNode->m_DynamicObjectList.begin();
