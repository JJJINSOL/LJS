...
#include "Input.h"
HRESULT QuadObject::CreateConstantBuffer(ID3D11Device* pd3dDevice)
{
	HRESULT hr;
...
bool QuadObject::SetBuffer(ID3D11Device* pd3dDevice)
{
...
void QuadObject::ComputeKernel(int iRadius )
{	
	float sigma = 1.0f;
	m_iMask = iRadius * 2 + 1;	
	...
	for (int x = 0; x < m_iMask; ++x) 
	{
		...
		{
			m_pfKernelMask[x*m_iMask+y] = expf( -0.5 * (pow((x-mean)/sigma, 2.0) + pow((y-mean)/sigma,2.0)) ) / (2 * 3.141592f * sigma * sigma);
...
HRESULT QuadObject::CreateTextures(ID3D11Device* pDevice, UINT iWidth, UINT iHeight)
{
	HRESULT hr = S_OK;
