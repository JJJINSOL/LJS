...
HRESULT DxRT::Create(ID3D11Device* pd3dDevice, FLOAT Width, FLOAT Height, DXGI_FORMAT dsTexFormat)
{
	HRESULT hr = S_OK;
...
HRESULT DxRT::UpdateDepthStencilView(ID3D11Device* pDevice, UINT Width, UINT Height, DXGI_FORMAT dsTexFormat)
{
	HRESULT hr;
...
void DxRT::Apply(ID3D11DeviceContext* pImmediateContext,ID3D11RenderTargetView* pRTV,ID3D11DepthStencilView* pDepthStencilView,
...
HRESULT DxRT::SaveFile(ID3D11DeviceContext* pContext, T_STR name)
{
	...
	if (m_pTexture != nullptr)
	{
		if (TextureMgr::SaveFile(pContext, m_pTexture.Get(), name))
		{
			return hr;
		}
	}
	else if (m_pSRV != nullptr)
	{
		...
		hr = TextureMgr::SaveFile(pContext, texture, name);
		texture->Release();
	}
	else if (m_pRenderTargetView != nullptr)
	{
		...
		hr = TextureMgr::SaveFile(pContext, texture.Get(), name + L"RTV");
	}
	return hr;
}
